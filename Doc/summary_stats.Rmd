...
```{r c3_fig, echo=FALSE, fig.height=3, fig.width=6, results='hide'}
fig_c3r
```


#### Summer
```{r c3_tbl_rxh, echo=FALSE, results='asis', fig.width=5, fig.height=6, fig.cap="Least square mean comparisons. Estimated mean ± 95%CI. There is a significant difference when arrows are not overlapped.", message=FALSE, fig.show='hold'}
lmer_anova_tbl(c3_by_rh_s_fin, caption = "C3 propotion in summer for rainfall and herbivore treatments")

lmer_anova_tbl(c3_s_m_fin, caption = "C3 propotion in summer for rainfall treatments")
plot(lsmeans::lsmeans(c3_s_m_fin, by = "year",spec = "treatment"), comparisons = TRUE)
```

```{r c3_tbl_r, echo=FALSE, results='asis', fig.width=5, fig.height=5, fig.cap="Least square mean comparisons. Estimated mean ± 95%CI. There is a significant difference when arrows are not overlapped.", message=FALSE, fig.show='hold'}
lmer_anova_tbl(c3_by_rh_w_fin, caption = "C3 propotion in winter for rainfall and herbivore treatments")

lmer_anova_tbl(c3_w_m_fin, caption = "C3 propotion in winter for rainfall treatments")
plot(lsmeans::lsmeans(c3_w_m_fin, by = "year",spec = "treatment"), comparisons = TRUE)
```


### One-way ANOVA for each harvest in each year
```{r oneway_anova_tbl, echo=FALSE, results='asis', warning=FALSE}
emphasize.strong.cells(cbind(which(summary_tbl[, 4] <= 0.05), 4))
emphasize.italics.cols(4)
