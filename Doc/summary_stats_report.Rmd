---
title: "DRIGRASS -Summary stats for report-"
author: "Shun Hasegawa"
date: "22 November 2016"
output:
  html_document:
    toc: yes
  word_document:
    toc: yes
---

```{r setup, include=FALSE}
library(car)
library(lmerTest)
library(ggplot2)
library(grid)
library(pander)
library(plyr)
library(dplyr)
library(tidyr)
library(vegan)
# knitr::opts_chunk$set(echo = TRUE, fig.path='Figs_report/DRIGRASS_harvest_',
#                       dev=c('pdf', 'png'), dpi = 600)
knitr::opts_chunk$set(echo = TRUE)
load("../Output/Data/all_obj.RData")
theme_set(theme_bw()) 


# function to generate anova table from lmer
lmer_anova_tbl <- function(lmermod, ...){
  a <- Anova(lmermod, test.statistic = "F")
  panderOptions('round', c(2, 0, 0, 3))
  if(any(a[, 4] <= 0.05)) emphasize.strong.cells(cbind(which(a[, 4] <= 0.05), 4))
  emphasize.italics.cols(4)
  pander(a,  justify = "right", ...)
}

```



## Introduction


## Summary stats


```{r smmry_stat_tbl, echo=FALSE, results='asis'}
sst <- ldply(all_ow_anova, function(x) x$summary_stats) %>%
  gather(term, pval, herb, treatment, `treatment:herb`) %>% 
  mutate(pval = ifelse(pval <= .05, as.character(get_star(pval, dagger = FALSE)), "ns"),
         term = recode_factor(term, treatment = "Rain", herb = "Herb", `treatment:herb` = "RxH"),
         SYT  = paste(year, term, sep = "\n")) %>%
  select(-year, -term) %>% 
  spread(SYT, pval) %>% 
  mutate(variable = factor(variable, levels = c("live", "Dead", "total", "c34ratios", 
                                                "grprop", "H", "J", "S", "Forks", "sr_ratio", 
                                      "Tips", "total_L", "total_SA"))) %>% 
  arrange(variable)

sst_summer <- sst %>% 
  filter(season == "Summer") %>% 
  select(-season) %>% 
  .[!apply(., 2, function(x) all(is.na(x)))]

sst_winter <- sst %>% 
  filter(season == "Winter") %>% 
  select(-season) %>% 
  .[!apply(., 2, function(x) all(is.na(x)))]


pander(sst_summer, split.cell = 2, split.table = Inf, caption = "Summary stats for summer harvests")
pander(sst_winter, split.cell = 2, split.table = Inf, caption = "Summary stats for winter harvests")

```



## Biomass (Total/Live/Dead)

```{r Fig1, echo=FALSE, fig.height=6.5, fig.width=6.5, results='hide'}
grid.draw(ab_biom_plot_merged)
```




## Diversity indices

```{r Fig2, echo=FALSE, fig.height=6.5, fig.width=6.5, results='hide'}
grid.draw(div_plot_merged)
```


## Plant functional groups


```{r Fig3, echo=FALSE, fig.height=6.5, fig.width=6.5, results='hide'}
grid.draw(pfg_fig_merged)
```




## MDS analysis

```{r Fig4a, echo=FALSE, fig.height=3.5, fig.width=6.5, results='hide', fig.cap="MDS for summer harvests"}
fig_pcoa_summer
```

```{r Fig4b, echo=FALSE, fig.height=3.5, fig.width=4.3, results='hide', fig.cap="MDS for winter harvests"}
fig_PCoA_winter
```




## Principal response curve (PRC) and PERMANOVA

```{r Fig5, echo=FALSE, fig.height=4, fig.width=6, results='asis', fig.cap="PRC for summer and winter harvests", fig.subcap=c("Summer", "Winter")}
get_prc_fig_smmr()
mtext(text = "Summer", side = 3)
get_prc_fig(prc_effect_wint, prc_sp_wint, ylim = c(-2, 2))
mtext(text = "Winter", side = 3)
```


```{r prc_rslt, echo=TRUE, results='markup'}
# Summer
prc_res_smmr

# Winter
prc_res_wint
```


```{r permanova, echo=FALSE, results='asis'}
emphasize.italics.cols(4)
emphasize.strong.cells(cbind(which(prmnv_res_tbl[,4] < .05), 4))
pander(prmnv_res_tbl, caption = "Results of PERMANOVA for each harvets for each year with 9999 permutations. The Bray-Curtis dissimilarity was used.")

```


## Community-level plant traits
Plant tratis do not seem to vary between rainfall treatments within the same species (specific root forks, tips, length and surface area and shoot:root ratios). In order to test if community-level plant traits were affected by the treatments, we computed abundance-weighted means of plant traits for each plot.

```{r Fig7, echo=FALSE, fig.height=8.5, fig.width=6.5, results='asis', fig.cap="Abundance-weighted means of communit-level plant tratis"}
grid.draw(trait_plot_merged)
```




## Fourth corner models (community assembly via trait selection; CATS)
```{r cats_tbl, echo=FALSE, results='asis'}
emphasize.strong.cells(cbind(which(rql_res[, 7] <= 0.05), 7))
emphasize.italics.cols(7)
pander(rql_res, caption = "Summary of results of forth coner models for each harvest")
```


```{r cat_plot, echo=FALSE, fig.width=4.5, fig.height=4.5}
l_ply(rql_plt_l, print)
```



## One-way ANOVA for each harvest in each year
```{r oneway_anova_tbl, echo=FALSE, results='asis', warning=FALSE}
emphasize.strong.cells(cbind(which(summary_tbl[, 5] <= 0.05), 5))
emphasize.italics.cols(5)
pander(summary_tbl, justify = "right", split.table = Inf, caption = "Results of one-way ANOVA and summary with mean±SE (n = 6 [control-herb] or 12[added- & control-herb]). Where there was a significant treatment effect at P < 0.05, post-hoc tests were performed. The results of post-hoc comparisons are shown by letters alongside parenthesis.")

```
